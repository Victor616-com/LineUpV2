...
const NoteBottomBar = forwardRef(({ noteId, onLike }, ref) => {
  ...
  const heartIcon = (
    <svg
      ...
      height="17"
      viewBox="0 0 19 17"
      fill={liked ? "#ffcf70" : "none"} // ❤️ filled if liked
    >
      <path
  ...
  // Fetch like count + whether the user liked this note
  useEffect(() => {
  ...
  const toggleLike = async () => {
    if (!session?.user) return alert("You must be logged in to like posts.");
  ...
  return (
    <div className="flex flex-row px-xs gap-5">
      {/* Likes */}
      <div
        className="flex flex-row items-center gap-xxs min-w-8" /*min with so that it doesn't shift around when changing small numbers*/
        onClick={toggleLike}
      >
        {heartIcon} <p className="text-xs ">{likeCount}</p>
      </div>
      {/* Comments */}
      <div className="flex flex-row items-center gap-xxs">
        {commentsIcon} <p className="text-xs ">0</p>
      </div>
      {/* Share */}
